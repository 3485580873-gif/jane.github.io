...
// ===== 信封点赞功能 =====
function toggleEnvLike(section, id) {
    const letters = section === 'outbox' ? envelopeData.outbox : envelopeData.inbox;
    const letter = letters.find(l => l.id === id);
    if (!letter) return;
    if (!letter.likes) letter.likes = [];
    const myId = 'user';
    const idx = letter.likes.indexOf(myId);
    if (idx > -1) {
        letter.likes.splice(idx, 1);
    } else {
        letter.likes.push(myId);
        // 梦角也有概率点赞
        if (typeof partnerPersonas !== 'undefined' && partnerPersonas && partnerPersonas.length > 0) {
            partnerPersonas.forEach((p, i) => {
                if (Math.random() < 0.5) {
                    const personaId = 'persona_' + i;
                    if (!letter.likes.includes(personaId)) {
                        letter.likes.push(personaId);
                    }
                }
            });
        }
    ...
    renderEnvelopeLists();
    // 如果正在查看这封信，也更新详情页
    if (editingEnvId === id) updateEnvViewLikeBtn(section, id);
}

function getEnvLikeCount(letter) {
    return (letter.likes && letter.likes.length) || 0;
}

function isEnvLikedByUser(letter) {
    return letter.likes && letter.likes.includes('user');
}

function getEnvLikeNames(letter) {
    if (!letter.likes || letter.likes.length === 0) return '';
    return letter.likes.map(uid => {
        if (uid === 'user') return (typeof settings !== 'undefined' && settings.myName) || '我';
        if (uid.startsWith('persona_') && typeof partnerPersonas !== 'undefined' && partnerPersonas) {
            const idx = parseInt(uid.replace('persona_', ''));
            return partnerPersonas[idx] ? partnerPersonas[idx].name : uid;
        }
        return uid;
    }).join('、');
}

function updateEnvViewLikeBtn(section, id) {
    const letters = section === 'outbox' ? envelopeData.outbox : envelopeData.inbox;
    const letter = letters.find(l => l.id === id);
    if (!letter) return;
    const likeBtn = document.getElementById('env-view-like-btn');
    const likeNames = document.getElementById('env-view-like-names');
    if (likeBtn) {
        const liked = isEnvLikedByUser(letter);
        const count = getEnvLikeCount(letter);
        likeBtn.innerHTML = liked 
            ? `<i class="fas fa-heart" style="color:var(--accent-color);margin-right:4px;"></i>已赞${count > 1 ? ' ' + count : ''}` 
            : `<i class="far fa-heart" style="margin-right:4px;"></i>赞${count > 0 ? ' ' + count : ''}`;
        likeBtn.className = 'env-view-action-btn' + (liked ? ' liked' : '');
    }
    if (likeNames) {
        const names = getEnvLikeNames(letter);
        likeNames.textContent = names ? names + ' 觉得很好' : '';
        likeNames.style.display = names ? 'block' : 'none';
    }
}
    ...
    list.innerHTML = envelopeData.outbox.slice().reverse().map(letter => {
        ...
        <div class="env-letter-item" onclick="viewEnvLetter('outbox','${letter.id}')">
            ...
            <div class="env-letter-body">
                ...
                <div class="env-letter-like" onclick="event.stopPropagation();toggleEnvLike('outbox','${letter.id}')">
                    <i class="${isEnvLikedByUser(letter) ? 'fas' : 'far'} fa-heart" style="${isEnvLikedByUser(letter) ? 'color:var(--accent-color);' : ''}"></i>
                    <span>${getEnvLikeCount(letter) || ''}</span>
                </div>
            </div>
...
function renderInboxList() {
    ...
    list.innerHTML = envelopeData.inbox.slice().reverse().map(letter => {
        ...
        <div class="env-letter-item reply ${isNew ? 'env-letter-new' : ''}" onclick="viewEnvLetter('inbox','${letter.id}')">
            ...
            <div class="env-letter-body">
                <div class="env-letter-preview">${preview}</div>
                <div class="env-letter-like" onclick="event.stopPropagation();toggleEnvLike('inbox','${letter.id}')">
                    <i class="${isEnvLikedByUser(letter) ? 'fas' : 'far'} fa-heart" style="${isEnvLikedByUser(letter) ? 'color:var(--accent-color);' : ''}"></i>
                    <span>${getEnvLikeCount(letter) || ''}</span>
                </div>
            </div>
...
window.viewEnvLetter = function(section, id) {
    ...
    // 初始化点赞状态
    updateEnvViewLikeBtn(section, id);
};
