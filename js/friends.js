...
(function() {
    ...
    async function checkPendingComments() {
        ...
        let changed = false;
        for (const post of posts) {
            if (!post.pendingComments || post.pendingComments.length === 0) continue;
            const now = Date.now();
            const stillPending = [];
            for (const pc of post.pendingComments) {
                const dueTime = pc.scheduledAt || pc.time;
                if (now >= dueTime) {
                    // 已过期，直接加入正式评论
                    post.comments.push({
                        id: pc.id,
                    ...
                    const commentId = pc.id;
                    setTimeout(async () => {
                        const curPosts = await getPosts();
                        const curPost = curPosts.find(p => p.id === postId);
                        if (curPost) {
                            if (curPost.pendingComments) {
                                curPost.pendingComments = curPost.pendingComments.filter(c => c.id !== commentId);
                            }
            ...
            if (stillPending.length !== post.pendingComments.length) {
                post.pendingComments = stillPending;
                changed = true;
            }
        }
        if (changed) {
    ...
    async function submitPost() {
        ...
        // 梦角100%概率自动评论，随机字卡组合，延迟5-10分钟
        if (typeof partnerPersonas !== 'undefined' && partnerPersonas && partnerPersonas.length > 0) {
            const personas = partnerPersonas;
            // 字卡池：不同类别的词组，随机组合成评论
            const wordCards = {
                openers: ['哇', '哎', '嗯', '嘿', '啊', '唔', '噢', '嗬', '哇哦', '哎哟'],
                feelings: ['好好看', '好开心', '好感动', '好喜欢', '好暖', '好棒', '好甜', '好帅', '好美', '好幸福', '好有趣', '好可爱'],
                phrases: ['看到这个心情都好了', '真的假的', '我也想要', '什么时候的事', '太厉害了吧', '每天都要这样', '可以可以', '记录生活真好', '跟你一样', '下次带我', '笑死我了', '突然好想哭', '我来了', '被我看到了', '这个必须赞', '懂了', '学到了', '收藏了', '羡慕了', '心动了'],
                emojis: ['😊', '❤️', '✨', '💕', '🎉', '😍', '🥰', '🤩', '😆', '🥺', '👍', '😘', '🌸', '💖', '😭', '🙏', '🫶', '💗', '🌈', '🤭'],
                closers: ['~', '！', '…', '呀', '呢', '啦', '嘛', '哈', '吧', '哒']
            };
            ...
            function composeComment() {
                ...
                // 随机选一种组合模式
                const patterns = [
                    () => rand(wordCards.openers) + rand(wordCards.feelings) + rand(wordCards.closers),
                    () => rand(wordCards.feelings) + rand(wordCards.emojis),
                    () => rand(wordCards.openers) + rand(wordCards.phrases) + rand(wordCards.closers),
                    () => rand(wordCards.phrases) + rand(wordCards.emojis),
                    () => rand(wordCards.feelings) + rand(wordCards.closers) + ' ' + rand(wordCards.phrases),
                    () => rand(wordCards.emojis) + rand(wordCards.feelings) + rand(wordCards.emojis),
                    () => rand(wordCards.openers) + rand(wordCards.openers) + rand(wordCards.feelings),
                    () => rand(wordCards.phrases),
                    () => rand(wordCards.feelings) + rand(wordCards.closers),
                    () => rand(wordCards.openers) + rand(wordCards.emojis) + ' ' + rand(wordCards.phrases)
                ];
                return patterns[Math.floor(Math.random() * patterns.length)]();
            }
            
            const savedPosts = await getPosts();
            const newPost = savedPosts.find(p => p.id === post.id);
            if (newPost) {
                if (!newPost.pendingComments) newPost.pendingComments = [];
                personas.forEach((persona, i) => {
                    const delayMinutes = 5 + Math.random() * 5; // 5-10分钟
                    const delayMs = Math.floor(delayMinutes * 60 * 1000);
                    const scheduledTime = Date.now() + delayMs;
                    const commentText = composeComment();
                    const commentData = {
                        id: 'comment_' + Date.now() + '_' + i + '_' + Math.random().toString(36).substr(2,4),
                        userId: 'persona_' + i,
                    ...
                    newPost.pendingComments.push(commentData);
                    // 定时添加评论
                    setTimeout(async () => {
                        const curPosts = await getPosts();
                        const curPost = curPosts.find(p => p.id === post.id);
                        if (curPost) {
                            if (curPost.pendingComments) {
                                curPost.pendingComments = curPost.pendingComments.filter(c => c.id !== commentData.id);
                            }
                            curPost.comments.push({
                                id: commentData.id,
    ...
    function createCommentOverlay() {
        ...
        commentOverlay.innerHTML = `
            <div class="friends-comment-input-bar">
                <div class="friends-comment-identity-bar" id="comment-identity-bar" style="display:flex;align-items:center;gap:6px;margin-bottom:8px;flex-wrap:wrap;"></div>
                <span class="friends-reply-label" style="display:none;position:absolute;top:-44px;left:16px;font-size:12px;color:var(--text-secondary);"></span>
    ...
    // ===== 渲染评论身份选择器 =====
    async function renderCommentIdentityBar() {
        const bar = document.getElementById('comment-identity-bar');
        if (!bar) return;
        
        const allUsers = await getAllUsers();
        let html = '<span style="font-size:11px;color:var(--text-secondary);margin-right:2px;">以</span>';
        for (const user of allUsers) {
            const isSelected = user.id === currentPublishIdentity;
            html += `
                <div class="friends-comment-identity-item ${isSelected ? 'selected' : ''}" data-uid="${user.id}" onclick="FriendsModule.setCommentIdentity('${user.id}')" style="display:flex;align-items:center;gap:4px;padding:3px 10px;border-radius:14px;cursor:pointer;font-size:12px;transition:all 0.2s;${isSelected ? 'background:rgba(var(--friends-accent-rgb),0.2);border:1px solid rgba(var(--friends-accent-rgb),0.4);color:var(--text-primary);font-weight:500;' : 'background:rgba(var(--friends-accent-rgb),0.06);border:1px solid transparent;color:var(--text-secondary);'}">
                    ${user.avatar ? `<img src="${user.avatar}" style="width:16px;height:16px;border-radius:50%;object-fit:cover;">` : '<i class="fas fa-user" style="font-size:10px;"></i>'}
                    <span>${user.name}</span>
                </div>
            `;
        }
    ...
    function setCommentIdentity(userId) {
        currentPublishIdentity = userId;
        renderCommentIdentityBar();
    }
    ...
    function addEntryToAdvanced() {
        ...
        function tryAddEntry() {
            ...
            if (list) {
                ...
                entry.innerHTML = `
                    <i class="fas fa-earth-americas" style="color: var(--friends-accent, var(--accent-color));"></i>
                    <span>朋友圈</span>
                `;
                entry.onclick = () => {
