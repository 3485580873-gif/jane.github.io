...
(function() {
    ...
    function getCurrentUser() {
        ...
        const motto = (typeof settings !== 'undefined' && settings.partnerName) ? settings.partnerName : '';
        return { id: 'user', name, avatar, motto };
    }

    function getPartnerUser() {
        const avatarEl = document.querySelector('#partner-avatar img');
        const avatar = avatarEl ? avatarEl.src : '';
        const name = (typeof settings !== 'undefined' && settings.partnerName) ? settings.partnerName : '梦角';
        return { id: 'partner', name, avatar };
    }
    ...
    function renderPost(post, user) {
        ...
        if (post.likes.length > 0) {
            const likeNames = post.likes.map(userId => {
                if (userId === 'user') return currentUser.name;
                if (userId === 'partner') return getPartnerUser().name;
                return userId;
            });
            likesHtml = `
            ...
            commentsHtml = `
                <div class="friends-comments">
                    ${post.comments.map(comment => {
                        const commentAuthor = comment.userId === 'user' ? currentUser :
                                             comment.userId === 'partner' ? getPartnerUser() : { name: comment.userName };
                        const replyHtml = comment.replyTo ? (() => {
                            const replyUser = comment.replyTo === 'user' ? currentUser :
                                            comment.replyTo === 'partner' ? getPartnerUser() : { name: '未知' };
                            return `<span class="friends-comment-reply">回复</span><span class="friends-comment-reply-name">${replyUser.name}</span>`;
                        })() : '';
        ...
        return `
            <div class="friends-post" data-post-id="${post.id}" style="animation-delay: ${Math.random() * 0.2}s">
                <div class="friends-post-header">
                    <div class="friends-post-avatar" onclick="FriendsModule.openFriendProfile('${post.userId}')">
                        ${post.userAvatar ? `<img src="${post.userAvatar}" alt="头像">` : '<i class="fas fa-user"></i>'}
                    </div>
                    <div class="friends-post-user">
                        <div class="friends-post-name">${post.userName}</div>
    ...
    async function renderFriendsList() {
        ...
        content.innerHTML = posts.map(post => renderPost(post, currentUser)).join('');
    }
                ...
                <div class="friends-header">
                    <div class="friends-header-left">
                        <div class="friends-avatar-cover">
                            ${currentUser.avatar ? `<img src="${currentUser.avatar}" alt="我的头像">` : '<i class="fas fa-user" style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;color:var(--text-secondary)"></i>'}
                        </div>
                        <div class="friends-user-info">
                            <span class="friends-user-name">${currentUser.name}</span>
                            <span class="friends-user-motto">${currentUser.motto || '记录美好瞬间'}</span>
                        </div>
                    </div>
    ...
    async function openPublish() {
        if (!publishModal) createPublishModal();
        ...
        publishModal.innerHTML = `
            <div class="friends-publish-modal">
                <div class="friends-publish-header">
                    <span class="friends-publish-title">发朋友圈</span>
    ...
    function previewImage(index) {
        ...
        const post = getPosts().then(async posts => {
            const p = posts.find(post => post.id === postId);
            if (p && p.images) {
    ...
    // ===== 更多菜单 =====
    function showMoreMenu(event, postId) {
        ...
        const post = getPosts().then(posts => {
            const p = posts.find(post => post.id === postId);
            const currentUser = getCurrentUser();
            const isOwner = p && p.userId === currentUser.id;
    ...
    function openFriendProfile(userId) {
        if (userId === 'user') {
            showToast('这是你自己');
        } else if (userId === 'partner') {
            showToast('查看' + getPartnerUser().name + '的资料');
        }
    }
